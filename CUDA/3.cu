...
__global__ void k_r_op(float * dop, const float * const dip, bool maxbool)
{
  ...
  const size_t noofbins)
{
  ...
  k_r_op << <bl_grp, varm, varm * sizeof(float) >> >(middle_i, d_LL, true);
  k_r_op << <1, bl_grp, bl_grp * sizeof(float) >> >(max_d, middle_i, true);
  k_r_op << <bl_grp, varm, varm * sizeof(float) >> >(middle_i, d_LL, false);
  k_r_op << <1, bl_grp, bl_grp * sizeof(float) >> >(min_d, middle_i, false);
